...
#    menuitems_table = pd.read_sql("SELECT * FROM menu ORDER BY orderid",connection)
...
def tables(menuID):
    try:
        pg_name = menuitems_table.loc[menuitems_table['orderid'] == menuID]['title']
...
def home_pg_list():
    try:
        tags = ""
        for title,id,func in zip(menuitems_table['title'],menuitems_table['orderid'],menuitems_table['func_name']):
...
def data(id):
    try:
        title = list(menuitems_table.loc[menuitems_table['orderid'] == id]['title'])[0]
        table_name = list(menuitems_table.loc[menuitems_table['orderid'] == id]['table_name'])[0]
        ...
        if('orderid' in list(table_data.columns)):
            table_data = pd.read_sql("SELECT * FROM "+table_name+'ODER BY orderid',connection)
